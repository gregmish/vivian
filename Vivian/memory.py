...
class MemoryManager:
    ...
    def _encrypt(self, data):
        # Simple reversible obfuscation (NOT secure encryption, for demonstration)
    ...
    def log_interaction(self, user_input, vivian_reply, mood=None, tags=None, sentiment=None, explanation=None, moderation_result=None, media=None):
        ...
        entry = {
            ...
            "tags": tags or [],
            "media": media  # Can be a dict: {"type": "image", "path": "..."}
        }
        with self.lock:
    ...
    def get_context(self, new_input=None, persona=None, language=None):
        """
        Returns a context window suitable for LLM prompt.
        Optionally includes a new user input and persona.
        """
        with self.lock:
    ...
    def remember_fact(self, fact_text, tags=None, shared=False):
        """
        Save a fact or note to the knowledge base for retrieval.
        Optionally shared for federated/distributed knowledge.
        """
    ...
    def search_knowledge(self, query, tags=None, shared_only=False):
        """
        Search saved facts/notes for keywords, optional tag and federated filter.
        """
        if not self.knowledge_base_enabled:
            return []
        matches = []
    ...
    def cleanup_old_entries(self):
        """
        Remove old log entries past retention period.
        """
    ...
    def get_stats(self):
        """
        Return analytics: total messages, sessions, facts, moods, etc.
        """
        stats = {
    ...
    # --- Multimodal support stubs (future extensibility) ---
    def save_media(self, media_type, data, tags=None):
        """
        Save multimodal data (image, audio, video) linked to memory.
        """
    ...
    # --- Federated/Distributed stubs ---
    def share_fact(self, fact_id):
        """
        Share a fact for federated/distributed knowledge base.
        """
        if not self.federated_knowledge_sharing:
            return "[Memory] Federated sharing not enabled."
        # Placeholder: implement sync to network
        return f"Fact {fact_id} marked for sharing (not yet implemented)."

    def sync_distributed(self):
        """
        Sync memory/knowledge with distributed nodes.
        """
        if not self.distributed_sync_enabled:
            return "[Memory] Distributed sync not enabled."
        # Placeholder: implement distributed sync
        return "[Memory] Sync complete (not yet implemented)."

    def get_recent(self, limit=10, user=None):
        """
        Get the most recent memory entries (optionally filtered by user).
        """
        with self.lock:
            if user:
                return [entry for entry in self.session if entry.get("user") == user][-limit:]
