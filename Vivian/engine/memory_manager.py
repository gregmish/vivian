...
class MemoryManager:
    """
    Ultra-advanced, self-evolving session/memory manager.
    Features:
    - Usage analytics, event timeline, tagging, search, multi-user, snapshots/versioning, retention, self-healing,
      notification/reminders, AI summarization, encryption (stub), plugin/event hooks, audit log export, multi-user/user switching, and more.
    """
    def __init__(self, user: str, notifier: Optional[Any] = None, llm: Optional[Any] = None, config: Optional[dict] = None, plugin_event_cb: Optional[Callable] = None):
        ...
        self._plugin_event_cb = plugin_event_cb  # Plugin hook callback
    ...
    def _encrypt(self, data: bytes) -> bytes:
        # STUB for encryption at rest (use real encryption if enabled in config)
        return data
    ...
    def load_memory(self):
        """Loads memory for current session, with self-healing and (optional) decryption."""
        with self.lock:
    ...
    def save_memory(self):
        """Saves current memory to file, with metadata, backup, snapshot, and (optional) encryption."""
        with self.lock:
            if self.memory_file.exists():
    ...
    def list_snapshots(self, session_name: Optional[str] = None) -> List[str]:
        """List all snapshots, optionally just for one session."""
    ...
    def session_usage_stats(self) -> Dict[str, int]:
        """Return a dict of session_name -> number of accesses."""
        return dict(sorted(
    ...
    def log_event(self, action: str, details: dict = None):
        self.memory.setdefault("_event_log", []).append({
            "time": current_time(),
            "action": action,
            "details": details or {},
        })
        self.save_memory()
        # Plugin hook
        self._plugin_event("on_log_event", {"action": action, "details": details})

    def session_timeline(self, session_name: Optional[str] = None) -> List[dict]:
        """Return event log for a session (or all)."""
        events = []
    ...
    def export_event_log(self, out_path: Path, all_sessions: bool = True):
        """Export all event logs for this user, all or current session."""
        all_events = []
    ...
    def list_sessions(self, details: bool = False) -> List[Dict[str, Any]]:
        """List all sessions for this user, with details if requested."""
    ...
    def switch_session(self, new_session: str) -> bool:
        """Switch to a new session (saves current, loads new)."""
        self.save_memory()
    ...
    def get_recent_context(self, limit: Optional[int] = None) -> List[dict]:
        ...
        if limit is None:
            # Optionally fetch from config/user stats if available
    ...
    def delete_session(self, session_name: str, archive: bool = True) -> bool:
        """Delete or archive a session file."""
    ...
    def summarize_session(self, session_name: Optional[str] = None) -> str:
        """Call LLM to summarize session context/notes."""
    ...
    def auto_suggest_cleanup(self):
        """If too many or old sessions, auto-suggest or trigger cleanup."""
        ...
        if len(sessions) > 30:
            ...
            # self.prune_sessions(keep_latest=20)
    ...
    def switch_user(self, new_user: str):
        """Switch to a new user (saves current, reloads meta/memory)."""
        self.save_memory()
        self.user = new_user
        self.meta_file = MEMORY_DIR / f"{self.user}{META_FILE_SUFFIX}"
        self.session_meta = {}
...
if __name__ == "__main__":
    ...
    print("Recent context:", mgr.get_recent_context())
